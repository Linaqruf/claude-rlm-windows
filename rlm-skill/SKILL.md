...
3. **Poll for progress** and keep the user informed. Check active sub-agents, completed results, and whether the orchestrator has finished:
   ...
   ls .rlm/$TASK/sessions/*/pid 2>/dev/null | while read f; do
       kill -0 "$(cat "$f")" 2>/dev/null && echo running
   done | wc -l
...
Sub-agents run as background processes. Monitor them via PID files and logs:

```bash
# List active sub-agents (by checking PIDs)
for f in .rlm/$TASK/sessions/*/pid; do
    pid=$(cat "$f" 2>/dev/null) && kill -0 "$pid" 2>/dev/null && echo "$(dirname "$f" | xargs basename): pid $pid"
done

# Check completed vs total
echo "$(find .rlm/$TASK/sessions -name '.done' | wc -l) done / $(ls .rlm/$TASK/sessions/ | wc -l) total"

# Tail a sub-agent's stderr log
